"""
is_field_issue now only triggers for L2 classifications (outside plant / physical layer) and four specific ISP codes — ISP-015, ISP-023, ISP-035, ISP-047 — all of which are physical/fiber issues requiring boots on the ground.
The old logic incorrectly included L4 (CPE/software — remote fixable) which has been removed.
The dispatch state is now explicitly set to False when it's not a field issue, preventing stale state from a previous analysis bleeding into a new one.
"""
    ...
    try:
        with st.spinner('Gemma [google/gemma-4-e4b] is processing...'):
            ...
            if "choices" in data:
                ...
                # Specific ISP codes are also flagged for critical physical issues:
                #   ISP-015: High Optical Loss / Dirty Connector
                #   ISP-023: Broken Drop Cable / Physical Cable Damage
                #   ISP-035: Sparking Wire / Induction on Messenger Wire (Critical)
                #   ISP-047: Low Signal / Splicing Joint Failure
