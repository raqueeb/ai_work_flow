"""
The AI is trained to read between the lines of customer complaints, identify the underlying issue, 
and route it to the correct department with a confidence score. 
This allows Link3 to provide lightning-fast resolutions and personalized service at scale.
"""
...
if analyze_btn and ticket_input.strip():
    ...
    try:
        with st.spinner('AI Model is routing...'):
            response = requests.post(LM_STUDIO_URL, json=payload, timeout=90)
            data = response.json()
            
            if "choices" in data:
                content = data["choices"][0]["message"]["content"]
                # Clean up any markdown blocks if the model includes them
...
with col2:
    ...
    if st.session_state.ai_result:
        ...
        # Display large metric for the department
        ...
        with st.container(border=True):
            ...
            st.progress(res.get("confidence", 0) / 100, 
                        text=f"Match Confidence: {res.get('confidence')}%")
